            ...
            <tr key={row.id || row.pk || i}>
              ...
              <td>
                <button
                  ...
                  data-bs-target="#modal-teams"
                  onClick={() => onDetails(row)}
                >
                  Details
                </button>
                <button type="button" className="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#edit-team-modal" onClick={() => { onEdit(row) }}>
                  Edit
                </button>
              </td>
            </tr>
  ...
  return (
    <div>
      ...
      <div className="mb-2 d-flex justify-content-end">
        <button className="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#create-team-modal">Create</button>
      </div>
      
      <div className="modal fade" id="create-team-modal" tabIndex="-1" aria-hidden="true">
          ...
          <form className="modal-content" onSubmit={async (e) => {
            ...
            try {
              ...
              if (!res.ok) {
                ...
                const el = document.getElementById('create-team-modal')
      ...
      <div className="modal fade" id="edit-team-modal" tabIndex="-1" aria-hidden="true">
        <div className="modal-dialog">
          <form key={editItem ? editItem.id : 'none'} className="modal-content" onSubmit={async (e) => {
            ...
            try {
              const res = await fetch(`${endpoint}${id}/`, {method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({name})})
              if (!res.ok) {
                let errMsg = `Request failed: ${res.status}`
                try { const body = await res.json(); errMsg = body.detail || JSON.stringify(body) } catch(e){ try{ errMsg = await res.text() }catch(_){} }
                setEditError(errMsg)
                ...
                const json = await res.json()
                console.log('Updated team', json)
                const el = document.getElementById('edit-team-modal')
                const modal = window.bootstrap && window.bootstrap.Modal.getInstance(el)
                if (modal) modal.hide()
                fetch(endpoint).then(r=>r.json()).then(j=>setData(Array.isArray(j)?j:j.results||[]))
              }
            ...
            <div className="modal-body">
              ...
              <div className="mb-3">
                <label className="form-label">Name</label>
                <input name="name" defaultValue={editItem?.name || ''} className="form-control" />
              </div>
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
      ...
      <div className="modal fade" id="modal-teams" tabIndex="-1" aria-hidden="true">
